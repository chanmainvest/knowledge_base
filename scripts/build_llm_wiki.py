...
- ``People/``    one page per *person* — interview guests, show hosts and
                 solo authors alike — opening with an LLM-written portrait
...
- ``Themes/``    cross-cutting theses, each opening with an LLM-written
                 overview of what the corpus says.
- ``Syntheses/`` the multi-dimensional views as essays: opinion shifts
                 (same person changing stance), disagreements (different
...
grounded in digest facts with ready-made citation links; the mechanical
tables below each narrative are the verbatim DB record. If the LLM is
unavailable, pages render without the narrative sections.
...
# ---------------------------------------------------------------------------
# Narrative prose layer (LLM-written, digest-hash cached)
#
# The mechanical tables further down each page are the verbatim DB record;
# the prose at the top is GLM writing from a compact digest of exactly
# those facts. The digest carries ready-made citation links so the prose
# can cite inline. Cache keyed by digest hash → unchanged data never
# re-calls the LLM.
# ---------------------------------------------------------------------------

WRITER_SYSTEM = """You are a human financial-editorial writer producing
wiki pages for an investment research corpus. You will receive a DIGEST of
extracted facts (calls, quotes, dates, stances) about one subject, each
with ready-made citation links.

Write flowing prose a sharp human analyst would write — NOT a dashboard.
Rules:

- 2-5 paragraphs of connected narrative; no bullet lists, no tables in
  your output.
- Weave names, dates, stances and specifics into sentences. Prefer
  concrete over generic ("turned bearish on gold by March" beats "changed
  views").
- Cite inline using the exact markdown links provided in the digest,
  right after the claim they support. Do not invent links or URLs.
- Use ONLY facts from the digest (plus widely-known context about public
  figures). Never fabricate quotes, numbers or calls. If the digest is
  thin, say so plainly and keep it short.
- Voice: direct, specific, a little opinionated about the *shape of the
  debate*, but never invent a verdict the data doesn't show. It is fine
  to note contradictions, hedging or thin coverage.
- Output ONLY the markdown prose (no headings, no preamble)."""
...
def load_prose_cache() -> None:
    global PROSE
    PROSE = {}
    if PROSE_CACHE.exists():
        try:
            PROSE = json.loads(PROSE_CACHE.read_text(encoding="utf-8"))
        except Exception:  # noqa: BLE001 — corrupt cache is regenerable
            PROSE = {}


def configure_prose(provider: str | None, model: str | None) -> None:
    global PROSE_PROVIDER, PROSE_MODEL
...
def _sanitize_links(text: str, path: Path) -> str:
    """Validate internal .md links in LLM prose against the actual tree.
    The writer is told to reuse the digest's ready-made links, but it
    sometimes renames them (david_woo vs david-woo). Try hyphen/underscore
    variants; failing that, keep the label and drop the dead link."""
    def _fix(m: re.Match) -> str:
        ...
        if target.exists():
            return m.group(0)
        alt = (path.parent / href.replace("_", "-")).resolve()
        if alt.exists():
            return f"[{label}]({href.replace('_', '-')})"
        alt2 = (path.parent / href.replace("-", "_")).resolve()
        if alt2.exists():
            return f"[{label}]({href.replace('-', '_')})"
...
def _quote_ref(path: Path, e: dict) -> str:
    """'date — stance: "quote" (link)' line for a digest."""
    ref = e["ref"]
    title = (ref.get("item_title") or "untitled").strip()
    label = title if len(title) <= 60 else title[:59] + "…"
    url = ref.get("item_url") or ref.get("url") or ""
    link = f"[{label}]({url})" if url else label
    return (f"{e['date']} {e['stance'].upper()}: "
            f"\"{truncate(e['quote'], 150)}\" ({link})")
...
def render_home(inv: dict, people_counts: dict[str, int],
    ...
    # --- state of the debate (LLM executive summary, digest-cached) ---------
    digest_lines = [f"COVERAGE: {t['n_extracted']:,} of {t['n_items']:,} items "
                    f"extracted; {inv['n_preds']:,} predictions, "
                    f"{inv['n_views']:,} market views, "
                    f"{len(PEOPLE)} people, {inv['n_tickers']} tickers"]
    top_people = sorted(PEOPLE.values(),
                        key=lambda p: p["n_events"], reverse=True)[:8]
    digest_lines.append("MOST-QUOTED PEOPLE: " + ", ".join(
        f"{p['name']} ({p['n_events']} calls)" for p in top_people))
    by_tk0 = group_predictions_by_ticker(inv["preds"])
    top_tk0 = sorted(by_tk0.items(), key=lambda kv: len(kv[1]), reverse=True)[:8]
    digest_lines.append("MOST-CALLED ASSETS: " + ", ".join(
        f"{tk} ({len(pl)})" for tk, pl in top_tk0))
    digest_lines.append("THEMES WITH DATA: " + ", ".join(
        th["title"] for th in THEMES
        if preds_for_theme(th["slug"]) or views_for_theme(th["slug"])))
    state = llm_prose(
        "home/state-of-debate",
        "Write the 'State of the debate' executive summary for the wiki "
        "home page: what this corpus of commentators is arguing about right "
        "now, the big divides, and the strongest conviction calls. Keep it "
        "grounded in the digest numbers; mention coverage honestly.",
        "\n".join(digest_lines), WIKI_DIR / "Home.md")
    if state:
        lines += ["## State of the debate", "", state, ""]
...
def render_ticker(ticker: str, plist: list[dict]) -> Path | None:
    ...
    # --- the debate (LLM narrative, digest-cached) --------------------------
    by_speaker: dict[str, list[dict]] = defaultdict(list)
    for p in plist:
        sp = resolve_person(p.get("speaker") or "") or p.get("speaker") or "?"
        by_speaker[sp].append(p)
    digest_lines = [f"ASSET: {ticker} ({asset_name})",
                    f"CONSENSUS: {consensus} "
                    f"({n_bull} bullish / {n_bear} bearish / {n_neut} neutral)"]
    digest_lines.append("CALLS BY SPEAKER (newest first):")
    for sp, plist2 in sorted(by_speaker.items(),
                             key=lambda kv: -len(kv[1]))[:8]:
        plink = person_link(path, sp) if sp in PEOPLE else sp
        digest_lines.append(f"- {plink}:")
        for p in plist2[:4]:
            q = truncate(p.get("quote"), 140)
            digest_lines.append(
                f"  * {fmt_date(p.get('made_at'))} {p.get('direction') or p.get('action')}: "
                f"\"{q}\"")
    debate = llm_prose(
        f"ticker/{ticker}",
        "Write 'the debate' around this asset for the wiki page: what the "
        "bulls argue, what the bears argue, who changed their mind, and "
        "what the timeline of calls looks like. Cite the people inline "
        "using the links in the digest.",
        "\n".join(digest_lines), path)
    if debate:
        lines += ["## The debate", "", debate, ""]
...
def render_person(person: dict, bios: dict[str, dict]) -> Path:
    ...
    # --- narrative portrait (LLM, digest-cached) ---------------------------
    digest_lines = [f"PERSON: {name}",
                    f"ROLES: {', '.join(role_bits) or 'unknown'}"]
    if bio.get("bio") or bio.get("role"):
        digest_lines.append(f"BACKGROUND: {bio.get('role') or ''} "
                            f"{bio.get('bio') or ''}".strip())
    digest_lines.append(f"SHOWS: {', '.join(person['channels'])}")
    digest_lines.append(f"STANCE MIX: {n_stances.get('bullish',0)} bullish / "
                        f"{n_stances.get('bearish',0)} bearish / "
                        f"{n_stances.get('neutral',0)} neutral")
    digest_lines.append("TOPIC TIMELINES (oldest → newest):")
    for t in topics[:8]:
        digest_lines.append(f"- {t['label']} (now {t['current']}"
                            + (", FLIPPED" if t["flips"] else "") + "):")
        for e in t["events"][:6]:
            digest_lines.append(f"  * {path}: {_quote_ref(path, e)}")
    if all_flips:
        digest_lines.append("STANCE CHANGES:")
        for t, (before, after) in all_flips[:6]:
            digest_lines.append(f"- {t['label']}: {before['date']} "
                                f"{before['stance']} -> {after['date']} "
                                f"{after['stance']}: "
                                f"\"{truncate(after['quote'], 120)}\"")
    portrait = llm_prose(
        f"person/{slug}",
        "Write the opening portrait of this person for their wiki page: who "
        "they are, what they argue for, and — if the timelines show it — how "
        "their views have moved over time. Name their strongest calls with "
        "dates and cite the linked items inline.",
        "\n".join(digest_lines), path)
    if portrait:
        lines += [portrait, ""]
...
def render_theme(theme: dict) -> Path:
    ...
    # --- overview narrative (LLM, digest-cached) ----------------------------
    digest_lines = [f"THEME: {theme['title']}",
                    f"SCALE: {len(preds)} tickered calls, {len(views)} broad views"]
    if tickers:
        digest_lines.append("TICKERS: " +
                            ", ".join(f"{tk} ({n})" for tk, n in tickers[:10]))
    digest_lines.append("NOTABLE CALLS:")
    for p in preds[:10]:
        sp = resolve_person(p.get("speaker") or "") or p.get("speaker") or "?"
        plink = person_link(path, sp) if sp in PEOPLE else sp
        digest_lines.append(f"- {plink} {fmt_date(p.get('made_at'))} "
                            f"{p.get('direction') or p.get('action')} "
                            f"{p.get('ticker') or p.get('asset_name')}: "
                            f"\"{truncate(p.get('quote'), 130)}\"")
    digest_lines.append("BROAD VIEWS:")
    for v in views[:8]:
        sp = resolve_person(v.get("speaker") or "") or v.get("speaker") or "?"
        plink = person_link(path, sp) if sp in PEOPLE else sp
        digest_lines.append(f"- {plink} {fmt_date(v.get('published_at'))} "
                            f"{v.get('direction')} {v.get('asset_class')}: "
                            f"\"{truncate(v.get('quote') or v.get('rationale'), 130)}\"")
    overview = llm_prose(
        f"theme/{slug}",
        "Write the overview essay for this theme page: what the corpus "
        "actually says about this theme, who the key voices are, where they "
        "agree and clash, and how the tone has shifted over time. Cite "
        "people inline using the links in the digest.",
        "\n".join(digest_lines), path)
    if overview:
        lines += ["## What the corpus says", "", overview, ""]
...
def render_opinion_shifts() -> Path:
    ...
    if not entries:
        ...
        digest_lines = ["STANCE FLIPS (same person, same topic, opposite "
                        "directions on different days):"]
        for name, t, (before, after) in entries[:15]:
            digest_lines.append(
                f"- {person_link(WIKI_DIR / 'x' / 'y.md', name)} on "
                f"{t['label']}: {before['date']} {before['stance']} "
                f"\"{truncate(before['quote'], 110)}\" -> {after['date']} "
                f"{after['stance']} \"{truncate(after['quote'], 110)}\"")
        essay = llm_prose(
            "synth/opinion-shifts",
            "Write the opening essay for the 'Opinion Shifts' page: what "
            "these reversals collectively say about how these commentators "
            "respond to changing markets. Group related flips, characterise "
            "who flips fast vs slow, and cite people inline via the digest "
            "links.",
            "\n".join(digest_lines), WIKI_DIR / "Syntheses" / "Opinion-Shifts.md")
        if essay:
            lines += [essay, ""]
...
def render_disagreements() -> Path:
    ...
    if not sections:
        ...
        digest_lines = ["DISAGREEMENTS (each person's latest directional "
                        "stance per topic):"]
        for t, rows in sections[:12]:
            digest_lines.append(f"- {t['label']}:")
            for n, s, e in rows[:8]:
                digest_lines.append(
                    f"  * {person_link(WIKI_DIR / 'x' / 'y.md', n)} {s} "
                    f"({e['date']}): \"{truncate(e['quote'], 110)}\"")
        essay = llm_prose(
            "synth/disagreements",
            "Write the opening essay for the 'Disagreements' page: the "
            "sharpest bull-vs-bear divides in this corpus and what each "
            "camp is really arguing about. Cite people inline via the "
            "digest links.",
            "\n".join(digest_lines), WIKI_DIR / "Syntheses" / "Disagreements.md")
        if essay:
            lines += [essay, ""]
...
def render_readme(inv: dict, counts: dict[str, int]) -> Path:
    ...
    lines = [
        ...
        "Verify anything load-bearing against the cited source items.",
...
def main() -> None:
    ...
    ap.add_argument("--no-prose", action="store_true",
                    help="skip LLM narrative sections (pages render with "
                         "mechanical sections only; cached prose is kept)")
    args = ap.parse_args()
    global PROSE_ENABLED
    PROSE_ENABLED = not args.no_prose
    ...
    load_prose_cache()
    configure_prose(args.provider, args.model)
